...
class SFTLayer(nn.Module):
    """Spatial Feature Transform: output = gamma(text_map) * features + beta(text_map)"""

    def __init__(self, feat_channels, reduction=4):
        super().__init__()
        mid = max(feat_channels // reduction, 4)
        self.gamma = nn.Sequential(
            nn.Conv2d(1, mid, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid, feat_channels, 3, padding=1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, feat, text_map):
        if text_map.shape[2:] != feat.shape[2:]:
            text_map = F.interpolate(text_map, size=feat.shape[2:],
                                     mode='bilinear', align_corners=False)
        return self.gamma(text_map) * feat + self.beta(text_map)
...
class Refiner(nn.Module):
    ...
    def forward(self, x, text_attention=None):
        inp_enc = self.patch_embed(x)

        inp_enc_level1_0 = self.down_1(inp_enc)
        if text_attention is not None and hasattr(self, 'sft_enc1'):
            inp_enc_level1_0 = self.sft_enc1(inp_enc_level1_0, text_attention)
        ...
        if text_attention is not None and hasattr(self, 'sft_enc2'):
            inp_enc_level2_0 = self.sft_enc2(inp_enc_level2_0, text_attention)
        ...
        if text_attention is not None and hasattr(self, 'sft_bottleneck'):
            out_agg_down_3 = self.sft_bottleneck(out_agg_down_3, text_attention)
        ...
        if text_attention is not None and hasattr(self, 'sft_dec2'):
            out_enc_level2_1 = self.sft_dec2(out_enc_level2_1, text_attention)
        ...
        if text_attention is not None and hasattr(self, 'sft_dec1'):
            out_enc_level1_1 = self.sft_dec1(out_enc_level1_1, text_attention)
